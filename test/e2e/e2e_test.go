...
var _ = Describe("External Secrets Operator End-to-End test scenarios", Ordered, func() {
	...
	Context("Proxy Egress Network Policy", Label("Platform:Generic", "Proxy:HTTP"), func() {
		AfterEach(func() {
			...
			By("Waiting for proxy egress policy to be removed after cleanup")
			Eventually(func(g Gomega) {
				_, err := clientset.NetworkingV1().NetworkPolicies(operandNamespace).Get(ctx, npProxyEgressPolicyName, metav1.GetOptions{})
				g.Expect(err).To(HaveOccurred(), "proxy egress policy should not exist after proxy config removed")
			}, 2*time.Minute, 5*time.Second).Should(Succeed())
		})

		It("should create proxy egress policy when proxy is configured with Managed provisioning", func() {
		...
		It("should not create proxy egress policy when provisioning is Unmanaged", func() {
